...
async def grader(request: Request):
    ...
    try:
        ...
        if session_id and session_id in env_sessions:
            ...
            # Normalize: map any total_reward > 0 into (0.01, 0.98)
            # Use a sigmoid-like normalization so 0 total never produces 0.0
            raw = total / (total + 1.0)  # always in [0, 1) for total >= 0
