...
class WordPuzzleEnvironment:
    ...
    def _compute_reward(self, guess: str, target: str, attempt_num: int) -> float:
        """
        Returns a reward STRICTLY between 0.01 and 0.98 (never 0.0 or 1.0).
        Solve bonus scales with how early the word was guessed.
        """
        if guess == target:
            remaining = self.max_attempts - attempt_num
            # Scale: earliest guess = 0.98, last guess = 0.50
            # Formula: 0.50 + 0.48 * (remaining / max_attempts)
            # When remaining=0  → 0.50
            # When remaining=max_attempts-1 (first guess) → 0.50 + 0.48*(n-1)/n < 0.98
            bonus = 0.50 + 0.48 * (remaining / self.max_attempts)
        ...
        # Partial match: clamp strictly to (0.01, 0.45)
        # Max possible raw score = 1.0 (all letters correct pos), but that
        # only happens when guess==target which is handled above.
        # So real max here is just under 1.0 for near-matches → cap at 0.45
        return round(min(0.45, max(0.01, score)), 4)
